...
const razorpay = new Razorpay({
    key_id: process.env.RAZORPAY_KEY_ID || 'rzp_test_o8p2O0C8C0RBGb ',
    key_secret: process.env.RAZORPAY_KEY_SECRET || 'uSPH0IE0wSB0SxIv5ihiIaIN '
});
...
app.get("/card/order-all-success", async(req, res) => {
    // Clear cart or save payment info
    // Example:
    await Cart.deleteMany({ user: req.user._id }); // if you're using a cart model
    res.send("✅ Order placed successfully and payment done!");
});
