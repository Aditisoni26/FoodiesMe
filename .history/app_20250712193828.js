...
app.post("/card/order-all", isLoggedIn, async(req, res) => {
    ...
    if (!scheduledAt || new Date(scheduledAt) < new Date()) {
        req.flash("error", "Please select a valid future delivery time.");
        return res.redirect("/card");
    }
    ...
    try {
        ...
        const orders = itemsInCart.map(item => {
            return new Order({
                ...
                category: item.category,
                user: req.user ? req.user._id : null,
                scheduledAt: new Date(scheduledAt) // ✅ Fix added
            });
        });
        ...
        req.flash("success", "All items successfully ordered!");
        res.redirect("/order");
    } catch (err) {
...
app.get("/verify-email", async(req, res) => {
    ...
    // Redirect to login page instead of plain message
    req.flash("success", "Email verified! Please log in.");
    res.redirect("/login");
});
app.get("/forgot-password", (req, res) => {
    res.render("auth/forgot-password"); // Make sure this view exists
});
...
app.post("/reset-password", async(req, res) => {
    ...
    try {
        await user.setPassword(password); // This auto-salts & hashes
...
app.post("/show/:id/add-review", isLoggedIn, async(req, res) => {
    try {
        const { id } = req.params;
...
app.get("/show/:id", async(req, res) => {
    const { id } = req.params;
    const item = await Listing.findById(id).populate("reviews.user");
    let user = null;
    if (req.user) {
        user = await User.findById(req.user._id).populate("wishlist");
    }
